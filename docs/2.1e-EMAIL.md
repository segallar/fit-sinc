...
> **Статус:** **2.1e.0 (infra) ✅** — `getsync/mail/`, Resend, `getsync mail test`, domain verified на prod · **2.1e (product) 📋** — M1/M2 verify, `/verify-email`, guard sync  
...
Единый способ **отправлять исходящую почту** из GetSync: подтверждение регистрации, позже — сброс пароля, invite, алерты. **Mailer в коде есть** (`getsync/mail/`, backends `null` / `console` / `resend`); **product flows** (verify при register, шаблоны, guard sync) — ещё **2.1e** / **2.6**. Дополнительно: bash [`scripts/check-getsync-dns.sh`](../scripts/check-getsync-dns.sh) (Gmail SMTP для ops DNS notify).
...
### Модуль (реализовано — 2.1e.0)

```text
getsync/mail/
  __init__.py          # send_email(), get_mailer()
  service.py           # MailSettings из env, routing backends
  backends/
    base.py
    resend.py          # HTTP API Resend
    console.py
    null.py
  templates/           # 📋 product flows (verify, reset)
```
...
|----|--------|--------|--------|
| 2.1e.0 | `getsync/mail/` + `MAIL_BACKEND=null` + Resend + тесты + `getsync mail test` | ½ вечера | ✅ |
| 2.1e.1 | Миграция: `email_verified`, `email_verify_token_hash`, `email_verify_sent_at` | ½ вечера | 📋 |
| 2.1e.2 | Prod DNS (SPF/DKIM) + Resend domain | ½ вечера | ✅ prod |
