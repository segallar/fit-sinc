...
> **ID roadmap:** **3.11** · подзадачи **3.11a** (FIT) · **3.11b** (wellness) · **3.11c** (UI)  
...
Web `JWT_WEB` может понадобиться для download-proxy — проверить в **spike 3.11.0**.
...
**Spike 3.11.0:** на реальном аккаунте проверить:
...
| **Dashboard** (новый блок или вкладка) | шаги сегодня / 7 дней, goal ring |
...
| ID | Задача | Оценка |
|----|--------|--------|
| **3.11.0** | Spike: download FIT + wellness API, OAuth vs JWT_WEB | ½–1 вечер |
| **3.11a.1** | `download.py`: ZIP→FIT, ошибки 404 | 1 вечер |
| **3.11a.2** | CLI `garmin download-fit`, `backfill-fit` | ½ вечера |
| **3.11a.3** | UI: download .fit для garmin rows | ½ вечера |
| **3.11a.4** | Тесты mock HTTP | ½ вечера |
| **3.11b.1** | Миграция `daily_steps`, `daily_sleep` | ½ вечера |
| **3.11b.2** | `wellness.py` + store + daily job | 1–2 вечера |
| **3.11b.3** | CLI `garmin sync-wellness --since` | ½ вечера |
| **3.11c.1** | Dashboard widget steps + sleep | 1–2 вечера |
| **3.11c.2** | Settings: «Sync wellness now», last fetched | ½ вечера |

**Итого:** ~5–8 вечеров (FIT-only ~2–3, wellness+UI ~3–5).

**Рекомендуемый порядок:** **3.11.0** → **3.11a** → **3.11b** → **3.11c**.
...
**Порядок в roadmap:** после стабилизации кабинета (**2.10** / **2.13**); параллельно с **2.8** spike возможен **3.11.0**.
