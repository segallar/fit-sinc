...
- [x] Домен **getsync.me** — регистрация (ожидание DNS / делегирования у регистратора).
- [ ] DNS: A/AAAA `getsync.me` и `app.getsync.me` → sirocco (после «ready for action»).
...
- [ ] certbot: `getsync.me`, `app.getsync.me`; новый [`deploy/nginx/getsync.conf`](../deploy/nginx/getsync.conf) (или расширить существующие).
...
**DNS (sirocco):** A/AAAA `@` и `app` → IP VPS; дождаться делегирования NS у регистратора.
