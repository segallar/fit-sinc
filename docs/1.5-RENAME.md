...
> **ID roadmap:** **1.5 C** (H1) · [PLAN.md](PLAN.md#15-c--cutover-getsyncme)  
> **Статус:** 🔄 **A+B в коде** (2026-05-26) · **C** — DNS/certbot/Hammerhead на prod · **документация** — [docs/README.md](README.md), [ARCHITECTURE.md](ARCHITECTURE.md), API/CI-CD обновлены под GetSync  
...
- [ ] **DNS вариант A (GoDaddy):** NS `ns37` / `ns38.domaincontrol.com` — [CI-CD.md § вариант A](CI-CD.md#вариант-a--godaddy-dns-основной)
- [ ] A `@` и `app` → `134.209.133.187` в GoDaddy DNS Records
- [ ] nginx [`getsync.conf`](../deploy/nginx/getsync.conf) + certbot на sirocco
- [ ] Hammerhead webhook / OAuth URI на `app.getsync.me`
- [ ] Проверить PyPI / trademark для `getsync` / `get-sync` (**R2–R3**).

### Этап 1 — Spike (½ вечера)

- [ ] `rg -i 'fit.sinc|fit_sinc|getsync|romansegalla'` — актуальный список (дополнить инвентарь выше).
- [ ] Выбран сценарий: **только A**, **A+B**, или **A+B+C**.
- [ ] План отката: тег git до rename, бэкап `/opt/getsync/data/`, snapshot DNS.
...
- [ ] `getsync.db`: на prod symlink `fit_sinc.db` → `getsync.db` при cutover (**C**).
...
- [ ] certbot: `getsync.me`, `app.getsync.me`; nginx — [`deploy/nginx/getsync.conf`](../deploy/nginx/getsync.conf)
- [ ] nginx: 301 с `fit.romansegalla.online` → новый host (минимум 6–12 мес).
- [ ] systemd: unit file, `Description`, `WorkingDirectory` (если R9).
- [x] GitHub: `segallar/getsync` → **`segallar/getsync`**; remote и badges в README.
...
- [ ] NS и A-записи по чеклисту CI-CD
