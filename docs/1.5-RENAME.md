...
> **Статус:** 📋 план · **имя продукта не утверждено**  
...
Сменить рабочее название **fit_sinc** на утверждённый бренд во всех слоях, где имя видно пользователю или влияет на интеграции — без потери данных и с предсказуемым деплоем.
...
|---|---------|----------|---------|
| R1 | **Публичное имя продукта** (UI, title, email) | TBD | — |
| R2 | **Slug / CLI** (`fit_sinc` → `???`) | `snake_case`, короткий, уникальный в PATH | — |
| R3 | **PyPI / package name** | `fit-sinc` → `???` (kebab) | — |
| R4 | **Домен приложения** | оставить `fit.romansegalla.online` / новый поддомен / один домен с лендингом | — |
| R5 | **Домен лендинга** | `romansegalla.online` без изменений / объединить с R4 | — |
| R6 | **GitHub repo** | переименовать `fit-sinc` / новый repo + redirect | — |
| R7 | **Cookie сессии** | новое имя + grace period со старым `fit_sinc_session` | — |
| R8 | **Файл SQLite** | оставить `fit_sinc.db` / переименовать + symlink | — |
| R9 | **Системный user / paths на VPS** | оставить `fit_sinc` /opt/fit_sinc` (только бренд в UI) / полный rename | — |
| R10 | **Hammerhead redirect URI** | обновить в dev portal при смене домена | — |

**Рекомендация v1 (минимальный риск):** R1–R3 + UI/docs + cookie с grace; **R4–R5** без смены DNS на первом релизе; **R9** оставить unix-user `fit_sinc` и `/opt/fit_sinc` до отдельного тикета.
...
- [ ] Таблица решений **R1–R10** заполнена (PR в этот файл или комментарий в issue).
- [ ] Проверена занятость домена / trademark / PyPI name для R2–R3.
...
- [ ] Новый каталог пакета `???/` (копия `fit_sinc/`).
...
- [ ] Cookie: `NEW_session` + middleware: принимать `fit_sinc_session` N дней → только новый.
- [ ] `fit_sinc.db`: переименовать на VPS + `DATA_DIR` / env, или symlink `fit_sinc.db` → `new.db`.

**Вариант B2 — только алиас CLI (не рекомендуется долго):**

- [ ] Новая команда `???` → тот же `cli:app`; `fit_sinc` deprecated warning.

**Критерий готовности:** `python -m unittest discover -s tests` зелёный; `??? --help` работает.

### Этап 4 — Уровень C (инфра)

- [ ] DNS: A/AAAA для нового `server_name` (если R4 меняется).
- [ ] certbot: выпустить сертификат; обновить [`deploy/nginx/fit.conf`](../deploy/nginx/fit.conf).
...
### Схема 1 — как сейчас (только бренд)
